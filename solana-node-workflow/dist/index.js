...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const customEndpoint = args?.endpoint || endpoint;
    try {
        switch (name) {
            ...
            case "classify_transaction": {
                const transactionData = args?.transactionData;
                const llmModel = args?.llmModel || "claude-3-opus-20240229";
                if (!transactionData) {
            ...
            case "analyze_with_expert": {
                const transactionData = args?.transactionData;
                const expertModel = args?.expertModel;
                const llmModel = args?.llmModel || "claude-3-opus-20240229";
                if (!transactionData) {
                    throw new Error("Transaction data is required");
                }
